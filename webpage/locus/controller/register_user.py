...
from locus.model.user import User
from locus.controller.login import login_required


@locus.route('/user/regist')
...
@locus.route('/user/regist', methods=['POST'])
def register_user():
    """포토로그 사용자 등록을 위한 함수"""
...
@locus.route('/user/<username>', methods=['POST'])
@login_required
...
@locus.route('/user/check_name', methods=['POST'])
def check_name():
