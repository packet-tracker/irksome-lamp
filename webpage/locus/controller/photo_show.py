...
from locus.database import dao
from locus.model.photo import Photo
from locus.controller.login import login_required
...
def __get_download_info(locus_id, prefix_filename=''):
    photo_info = get_photo_info(locus_id)
...
@locus.route('/photo/download/<locus_id>')
@login_required
def download_photo(locus_id):
    return __get_download_info(locus_id)


@locus.route('/photo/thumbnail/<locus_id>')
@login_required
def download_thumbnail(locus_id):
    return __get_download_info(locus_id, 'thumb_')



@locus.route('/photo/', defaults={'page': 1})
@locus.route('/photo/page/<int:page>')
@login_required
def show_all(page=1):    
...
@locus.route('/photo/search', methods=['POST'])
@login_required
def search_photo():    
